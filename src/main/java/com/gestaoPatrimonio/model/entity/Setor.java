...
import java.util.List;
    ...
    public class Setor {
        ...
        private void registrarBem(Bem bem) {
            // Adiciona o bem à lista de bens do setor
            this.bens.add(bem);
        }
        ...
        public void receberBem(Bem bem) {
            if (bem == null) {
                throw new IllegalArgumentException("Bem não pode ser nulo.");
            }
        
            bem.setResponsavel(this.getResponsavel());
        
            bem.setLocalizacao(this.getLocalizacao());
        
            bem.setSituacao("Alocado");
        
            this.registrarBem(bem);
        
            System.out.println("Bem " + bem.getDescricao() + " recebido no setor " + this.getNome());
        }
        ...
        public void devolverBem(Bem bem) {
            ...
            if (!this.bens.contains(bem)) {
                throw new IllegalArgumentException("Bem não pertence a este setor.");
            }

            this.bens.remove(bem);

            bem.setSituacao("Disponível");

            bem.setLocalizacao(null);
            bem.setResponsavel(null);

            System.out.println("Bem " + bem.getDescricao() + " devolvido do setor " + this.getNome());
        }
    }
